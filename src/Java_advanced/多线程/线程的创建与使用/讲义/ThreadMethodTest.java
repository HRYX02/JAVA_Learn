package Java_advanced.多线程.线程的创建与使用.讲义;

/**
 *测试Thread中的常用方法：
 * 1 start(): 启动当前线程；调用当前线程的run()
 * 2 run():通常需要重写Thread类中的此方法，将创建的线程要执行的操作声明在此方法中
 * 3 currentThread():静态方法，返回执行当前代码的线程
 * 4 getName():获取当前线程的名字
 * 5 setName():设置当前线程的名字
 * 6 yield():释放当前cpu的执行权
 * 7 join():在线程A中调用线程B的join()，此时线程A就进入阻塞状态，
 *          直到线程B完全执行完之后，线程A才结束阻塞状态
 * 8 st0P():已过时。当执行此方法时，强制结束当前线程。
 * 9 sleep(Long millitime):让当前线程"睡眠"指定的millitime毫秒。
 *                         在指定millitime毫秒时间内，当前线程是阻塞状态。
 * 10 isAlive():判断当前线程是否存活
 *
 * 线程的优先级：
 * 1
 * MAX_PRIORITY:10
 * MIN_PRIORITY:1
 * NORM_PRIORITY:5  -->默认优先级
 * 2.如何获取和设置当前线程的优先级：
 *      getPriority()：获取线程的优先级
 *      setPriority(int p)：设置线程的优先级
 *      说明：高优先级的线程要抢占低优先级线程u的执行权。但是只是从概率上讲，高优先级的线程高概率的情况下
 *            被执行。并不意味只有当高优先级的线程执行完以后，低优先级的线程才执行。
 * @author SHIXINXI
 * @create 2022-06-09-19:28
 */
public class ThreadMethodTest {
    public static void main(String[] args) {
        HelloTread helloTread = new HelloTread("线程：1");
        helloTread.setName("线程一");
        helloTread.start();
        Thread.currentThread().setName("主线程");
        System.out.println(helloTread.isAlive());
        for (int i = 0; i < 100; i++) {
            if (i % 2 != 0) {
                System.out.println(Thread.currentThread().getName() + ":" + i);
            }
            if (i == 20){
                try {
                    helloTread.join();
                }catch (InterruptedException e){
                    e.getMessage();
                }
            }
        }
        System.out.println(helloTread.isAlive());
    }
}
class HelloTread extends Thread{
    public HelloTread(String name){
        super(name);
    }
    /**
     * 为什么不抛异常？
     * 因为父类中的run()没有抛异常，子类重写方法必须小于等于父类方法的异常*/
    @Override
    public void run() {
        for (int i = 0; i < 100; i++) {
            if (i % 2 == 0) {
                try {
                    sleep(10);
                } catch (InterruptedException e) {
                    throw new RuntimeException(e);
                }
                System.out.println(Thread.currentThread().getName() + ":" + i);
            }
            if (i % 20 ==0){
                super.yield( );
            }
        }
    }
}

